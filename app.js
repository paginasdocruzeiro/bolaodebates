...
function normalizeAutomationSettings(raw = {}) {
  ...
  return {
    ...
    openBetsEnabled: source.openBetsEnabled !== false,
    closeBetsEnabled: source.closeBetsEnabled !== false,
    autoApplyResults: source.autoApplyResults !== false,
    autoCreateRounds: source.autoCreateRounds !== false,
    allowManualOverride: source.allowManualOverride !== false,
    openBeforeValue: Number.isFinite(Number(source.openBeforeValue)) ? Number(source.openBeforeValue) : DEFAULT_AUTOMATION_SETTINGS.openBeforeValue,
    closeBeforeValue: Number.isFinite(Number(source.closeBeforeValue)) ? Number(source.closeBeforeValue) : DEFAULT_AUTOMATION_SETTINGS.closeBeforeValue,
    openBeforeUnit: ['minutes', 'hours', 'days'].includes(source.openBeforeUnit) ? source.openBeforeUnit : DEFAULT_AUTOMATION_SETTINGS.openBeforeUnit,
    closeBeforeUnit: ['minutes', 'hours', 'days'].includes(source.closeBeforeUnit) ? source.closeBeforeUnit : DEFAULT_AUTOMATION_SETTINGS.closeBeforeUnit,
    newRoundDefaultMode: ['auto', 'upcoming', 'open', 'closed'].includes(source.newRoundDefaultMode) ? source.newRoundDefaultMode : DEFAULT_AUTOMATION_SETTINGS.newRoundDefaultMode
  };
}
...
function ensureStateSettings() {
  ...
  state.settings.automation = normalizeAutomationSettings(state.settings.automation || state.automation || DEFAULT_AUTOMATION_SETTINGS);
}

function automationUnitMs(value, unit) {
...
function computeAutomationTimes(matchTime, settings = getAutomationSettings()) {
  const matchMs = parseAppDateTime(matchTime);
  if (!matchMs || Number.isNaN(matchMs)) return { autoOpenAt: '', deadline: '' };

  return {
    autoOpenAt: toLocalInputInAppTime(new Date(matchMs - automationUnitMs(settings.openBeforeValue, settings.openBeforeUnit))),
    deadline: toLocalInputInAppTime(new Date(matchMs - automationUnitMs(settings.closeBeforeValue, settings.closeBeforeUnit)))
  };
}
...
function nextRoundNumber() {
  ...
  return rounds.reduce((max, round) => {
    ...
    if (Number.isFinite(explicit)) return Math.max(max, explicit);
...
/* === BOLAO CLEAN REPAIR V6 === */
(function installCleanRepairV6() {
  const originalBuildInitialState = buildInitialState;
  buildInitialState = function buildInitialStateV6() {
    const base = originalBuildInitialState();
    base.settings = {
      ...(base.settings || {}),
  ...
  normalizeState = function normalizeStateV6(raw) {
    const normalized = originalNormalizeState(raw);
    normalized.settings = {
      ...(raw?.settings && typeof raw.settings === 'object' ? raw.settings : {}),
      automation: normalizeAutomationSettings(
        raw?.settings?.automation ||
        raw?.automation ||
        DEFAULT_AUTOMATION_SETTINGS
      )
    };
  ...
  const originalPersistLocalState = persistLocalState;
  persistLocalState = function persistLocalStateV6() {
    ensureStateSettings();
    originalPersistLocalState();
  };

  saveState = function saveStateV6(scope = 'all') {
    applyAdminFlags();
    ensureStateSettings();
    ...
    if (scope === 'bets') return Promise.resolve();
    ...
    return writePromise.catch(err => {
      console.error('[Firebase] saveState falhou:', err);
      showToast('Erro ao salvar no Firebase: ' + (err?.code || err?.message || err));
    });
  };

  effectiveRoundState = function effectiveRoundStateV6(round) {
    if (!round) return 'none';
    ...
    const nowMs = Date.now();
    const openMs = matchMs - automationUnitMs(settings.openBeforeValue, settings.openBeforeUnit);
    const deadlineMs = matchMs - automationUnitMs(settings.closeBeforeValue, settings.closeBeforeUnit);

    if (settings.closeBetsEnabled && nowMs >= deadlineMs) return 'closed';
    if (!settings.openBetsEnabled) return 'upcoming';
    if (nowMs < openMs) return 'upcoming';

    return 'open';
  };

  getBetsArray = function getBetsArrayV6() {
    if (!state?.bets) return [];

    const latestByUserRound = new Map();

    Object.values(state.bets).filter(Boolean).forEach(bet => {
      const key = `${bet.roundId}::${bet.userName}`;
      const existing = latestByUserRound.get(key);
      const betTime = Date.parse(bet.updatedAt || bet.createdAt || 0);
      const existingTime = Date.parse(existing?.updatedAt || existing?.createdAt || 0);

      if (!existing || betTime >= existingTime) {
        latestByUserRound.set(key, bet);
      }
    });

    return Array.from(latestByUserRound.values());
  };

  getBet = function getBetV6(roundId, userName) {
    return getBetsArray()
      .filter(b => b.roundId === roundId && b.userName === userName)
      .sort((a, b) => Date.parse(b.updatedAt || b.createdAt || 0) - Date.parse(a.updatedAt || a.createdAt || 0))[0] || null;
  };

  async function writeBetWithFallback(betId, updatedBet, existing, firebaseUid) {
    if (!firebaseDbRef) return { id: betId, bet: updatedBet, mode: 'local' };

    try {
      await firebaseDbRef.child('bets').child(betId).set(updatedBet);
      return { id: betId, bet: updatedBet, mode: 'updated' };
    } catch (err) {
      console.warn('[Firebase] atualização da aposta falhou:', err);

      const permissionDenied = String(err?.code || err?.message || '').toUpperCase().includes('PERMISSION');

      if (!permissionDenied || !existing) {
        throw err;
      }
      ...
      const fallbackBet = {
        ...
        id: fallbackId,
        firebaseUid: firebaseUid || null,
        migratedFromBetId: existing.id || betId,
        createdAt: updatedBet.createdAt || new Date().toISOString(),
        updatedAt: new Date().toISOString()
      };

      await firebaseDbRef.child('bets').child(fallbackId).set(fallbackBet);
      return { id: fallbackId, bet: fallbackBet, mode: 'created-fallback' };
    }
  }

  upsertBet = function upsertBetV6({ roundId, userName, cruzeiroGoals, opponentGoals }) {
    const nowIso = new Date().toISOString();
    const existing = getBet(roundId, userName);
    const loggedUser = currentUser();
    const firebaseUid = getFirebaseUid();
    ...
    writeBetWithFallback(betId, updatedBet, existing, firebaseUid)
      .then(result => {
        if (result.id !== betId) {
          delete state.bets[betId];
          state.bets[result.id] = result.bet;
          persistLocalState();
        }

        showToast(existing ? 'Palpite atualizado com sucesso.' : 'Palpite registrado com sucesso.');
        renderAll('round');
      })
      .catch(err => {
        console.error('Erro ao guardar aposta no Firebase:', err);
        showToast('Erro ao salvar no servidor: ' + (err?.code || err?.message || err));
      });
  };

  importUpcomingMatch = function importUpcomingMatchV6(match) {
    const settings = getAutomationSettings();
    const localMatchTime = matchToLocalInput(match);
    const times = computeAutomationTimes(localMatchTime, settings);
    ...
    const round = {
      ...
      resultCruzeiro: null,
      resultOpponent: null,
      manualState: settings.newRoundDefaultMode || 'auto',
  ...
  function ensureAutomationStyles() {
    if (el('automationSettingsStyles')) return;

    const style = document.createElement('style');
    style.id = 'automationSettingsStyles';
    style.textContent = `
      ...
      .admin-auto-status-ribbon.open { border-color: rgba(57,217,138,.42); background: rgba(57,217,138,.11); }
      .admin-auto-status-ribbon.open .admin-auto-status-title { color: var(--green); }
      .admin-auto-status-ribbon.closed { border-color: rgba(255,215,107,.42); background: rgba(255,215,107,.10); }
      .admin-auto-status-ribbon.closed .admin-auto-status-title { color: var(--yellow); }
      .admin-auto-status-ribbon.waiting { border-color: rgba(96,165,250,.42); background: rgba(59,130,246,.10); }
      .admin-auto-status-ribbon.waiting .admin-auto-status-title { color: var(--accent-soft); }
      .automation-settings-grid { display:grid; gap:16px; max-width:980px; }
      ...
      .automation-card-title span { color:var(--text-2); font-size:.84rem; }
      .automation-toggle-row { display:flex; align-items:center; justify-content:space-between; gap:18px; padding:12px 0; }
      .automation-toggle-text { display:grid; gap:3px; }
      .automation-toggle-text span { font-size:.82rem; color:var(--text-2); }
      .automation-toggle-row input[type="checkbox"] { width:18px; height:18px; flex:0 0 auto; }
      .automation-row { display:grid; grid-template-columns:minmax(180px,1fr) 160px auto; gap:12px; align-items:end; margin-top:12px; }
      .automation-field { display:grid; gap:7px; color:var(--text-2); font-size:.84rem; }
      .automation-summary {
        padding:15px 17px; border-radius:16px; border:1px solid rgba(96,165,250,.35);
        background:rgba(59,130,246,.09); color:var(--text-2); line-height:1.55; max-width:980px;
      }
      .automation-actions { display:flex; gap:10px; flex-wrap:wrap; }
      @media (max-width:720px) { .automation-row { grid-template-columns:1fr; } }
    `;
  ...
  renderAdminRoundStatusRibbon = function renderAdminRoundStatusRibbonV6(round = getRound(el('roundSelect')?.value)) {
    ensureAutomationStyles();

    const manualSelect = el('roundManualState');
    if (!manualSelect || !round) return;

    let ribbon = el('adminRoundAutoStatusRibbon');
    if (!ribbon) {
    ...
    const settings = getAutomationSettings();
    const matchTime = el('roundMatchTime')?.value || round.matchTime;
    const manualState = el('roundManualState')?.value || round.manualState || 'auto';
    const resultC = el('resultCruzeiro')?.value === '' ? round.resultCruzeiro : Number(el('resultCruzeiro')?.value);
    const resultO = el('resultOpponent')?.value === '' ? round.resultOpponent : Number(el('resultOpponent')?.value);
    const hasResult = resultC !== null && resultC !== undefined && resultO !== null && resultO !== undefined;
    ...
    if (hasResult) {
      ...
      desc = `Cruzeiro ${resultC} x ${resultO} ${round.opponent || ''}.`;
    } else if (settings.allowManualOverride !== false && manualState && manualState !== 'auto') {
      const labels = { upcoming: 'Em espera', open: 'Apostas abertas', closed: 'Apostas encerradas', finalized: 'Rodada finalizada' };
      cls = manualState === 'open' ? 'open' : manualState === 'closed' ? 'closed' : 'waiting';
      title = 'Estado manual: ' + (labels[manualState] || manualState);
      desc = 'Para usar a automação, escolha “Automático” e salve.';
    } else {
      const matchMs = parseAppDateTime(matchTime);
      const openMs = matchMs - automationUnitMs(settings.openBeforeValue, settings.openBeforeUnit);
      const closeMs = matchMs - automationUnitMs(settings.closeBeforeValue, settings.closeBeforeUnit);
      const nowMs = Date.now();

      if (!settings.enabled) {
        cls = 'waiting';
        title = 'Automação desligada';
        desc = 'A rodada depende do controlo manual.';
      } else if (!matchMs || Number.isNaN(matchMs)) {
        cls = 'waiting';
        title = 'Data do jogo ausente';
        desc = 'Sem data/hora válida.';
      } else if (settings.closeBetsEnabled && nowMs >= closeMs) {
        cls = 'closed';
        title = 'Apostas encerradas';
        desc = 'Aguardando resultado oficial da API ou lançamento manual.';
      } else if (!settings.openBetsEnabled) {
        cls = 'waiting';
        title = 'Abertura automática desligada';
        desc = 'As apostas só abrem manualmente.';
      } else if (nowMs < openMs) {
        cls = 'waiting';
        title = 'Apostas abrem em ' + formatDurationShort(openMs - nowMs);
        desc = `Abertura: ${formatDateTime(toLocalInputInAppTime(new Date(openMs)))}. Fecho: ${formatDateTime(toLocalInputInAppTime(new Date(closeMs)))}.`;
      } else {
        cls = 'open';
        title = 'Apostas abertas';
        desc = settings.closeBetsEnabled
          ? `Fecham em ${formatDurationShort(closeMs - nowMs)}, às ${formatDateTime(toLocalInputInAppTime(new Date(closeMs)))}.`
          : 'Fecho automático desligado.';
      }
    }

    ribbon.className = `admin-auto-status-ribbon ${cls}`;
    ribbon.innerHTML = `
      <div class="admin-auto-status-title">${title}</div>
      <div class="admin-auto-status-desc">${desc}</div>
    `;
  };

  const originalPopulateRoundForm = populateRoundForm;
  populateRoundForm = function populateRoundFormV6(roundId) {
    originalPopulateRoundForm(roundId);
    const round = getRound(roundId);
    if (!round) return;

    if ((round.manualState || 'auto') === 'auto') {
      const times = computeAutomationTimes(round.matchTime);
      if (times.deadline && el('roundDeadline')) el('roundDeadline').value = times.deadline;
    }

    renderAdminRoundStatusRibbon(round);
  };
  ...
  function settingsSummary(settings) {
    const label = unit => unit === 'hours' ? 'hora(s)' : unit === 'days' ? 'dia(s)' : 'minuto(s)';
    if (!settings.enabled) return 'A automação está desligada.';
    return [
      settings.openBetsEnabled ? `As apostas abrem ${settings.openBeforeValue} ${label(settings.openBeforeUnit)} antes do jogo.` : 'A abertura automática está desligada.',
      settings.closeBetsEnabled ? `As apostas fecham ${settings.closeBeforeValue} ${label(settings.closeBeforeUnit)} antes do jogo.` : 'O fecho automático está desligado.',
      settings.autoApplyResults ? 'Resultados são aplicados automaticamente quando a API confirmar jogo finalizado.' : 'Resultados serão manuais.',
      settings.autoCreateRounds ? `Novas rodadas são criadas automaticamente em modo “${settings.newRoundDefaultMode}”.` : 'Criação automática de rodadas desligada.'
    ].join('<br>');
  }
  ...
  function renderAutomationSettingsPanel() {
    ...
    const content = el('adminContent') || el('view-admin');
    const tabBar = content?.querySelector('.tab-bar');
    if (!tabBar) return;

    if (!el('tab-admin-settings-button')) {
    ...
    let pane = el('tab-admin-settings');
    if (!pane) {
      pane = document.createElement('div');
      pane.id = 'tab-admin-settings';
      pane.className = 'tab-pane hidden';
      (tabBar.parentElement || content).appendChild(pane);
    }
      ...
      <section class="panel">
        ...
        <form id="automationSettingsForm" class="automation-settings-grid">
          <div class="automation-settings-card">
            <div class="automation-card-title"><strong>Automação geral</strong><span>Controla todo o comportamento automático</span></div>
            <div class="automation-toggle-row">
              <div class="automation-toggle-text"><strong>Ativar automação das rodadas</strong><span>Quando desligado, as rodadas dependem do controlo manual.</span></div>
              <input type="checkbox" id="autoEnabled" ${settings.enabled ? 'checked' : ''}>
            </div>
          </div>

          <div class="automation-settings-card">
            <div class="automation-card-title"><strong>Janela de apostas</strong><span>Abertura e fecho calculados a partir do horário do jogo</span></div>

            <div class="automation-toggle-row">
              <div class="automation-toggle-text"><strong>Abrir apostas automaticamente</strong><span>Exemplo: 24 horas antes do jogo.</span></div>
              <input type="checkbox" id="openBetsEnabled" ${settings.openBetsEnabled ? 'checked' : ''}>
            </div>

            <div class="automation-row">
              <label class="automation-field">Abrir apostas<input type="number" id="openBeforeValue" min="0" step="1" value="${settings.openBeforeValue}"></label>
              <label class="automation-field">Unidade<select id="openBeforeUnit">${unitOptions(settings.openBeforeUnit)}</select></label>
              <span class="automation-hint">antes do jogo</span>
            </div>

            <div class="automation-toggle-row" style="margin-top:12px;">
              <div class="automation-toggle-text"><strong>Fechar apostas automaticamente</strong><span>Exemplo: 5 minutos antes do jogo.</span></div>
              <input type="checkbox" id="closeBetsEnabled" ${settings.closeBetsEnabled ? 'checked' : ''}>
            </div>

            <div class="automation-row">
              <label class="automation-field">Fechar apostas<input type="number" id="closeBeforeValue" min="0" step="1" value="${settings.closeBeforeValue}"></label>
              <label class="automation-field">Unidade<select id="closeBeforeUnit">${unitOptions(settings.closeBeforeUnit)}</select></label>
              <span class="automation-hint">antes do jogo</span>
            </div>
          </div>

          <div class="automation-settings-card">
            <div class="automation-card-title"><strong>Resultados e importação</strong><span>Como a API alimenta o bolão</span></div>
            <div class="automation-toggle-row">
              <div class="automation-toggle-text"><strong>Aplicar resultado automaticamente</strong><span>Usa o placar final quando a API indicar jogo finalizado.</span></div>
              <input type="checkbox" id="autoApplyResults" ${settings.autoApplyResults ? 'checked' : ''}>
            </div>
            <div class="automation-toggle-row">
              <div class="automation-toggle-text"><strong>Criar rodadas automaticamente</strong><span>Próximos jogos da API viram novas rodadas.</span></div>
              <input type="checkbox" id="autoCreateRounds" ${settings.autoCreateRounds ? 'checked' : ''}>
            </div>
            <label class="automation-field" style="margin-top:14px;">Estado padrão das novas rodadas
          ...
          <div class="automation-settings-card">
            <div class="automation-card-title"><strong>Controlo manual</strong><span>Permite exceções rodada a rodada</span></div>
            <div class="automation-toggle-row">
              <div class="automation-toggle-text"><strong>Permitir override manual</strong><span>Admins podem forçar “Apostas abertas”, “Encerradas” ou “Finalizada”.</span></div>
              <input type="checkbox" id="allowManualOverride" ${settings.allowManualOverride !== false ? 'checked' : ''}>
            </div>
          </div>

          <div class="automation-summary" id="automationSettingsSummary">${settingsSummary(settings)}</div>
    ...
    function readSettingsForm() {
      return normalizeAutomationSettings({
        ...
        openBetsEnabled: !!el('openBetsEnabled')?.checked,
        openBeforeValue: Number(el('openBeforeValue')?.value),
        openBeforeUnit: el('openBeforeUnit')?.value,
        closeBetsEnabled: !!el('closeBetsEnabled')?.checked,
        closeBeforeValue: Number(el('closeBeforeValue')?.value),
        closeBeforeUnit: el('closeBeforeUnit')?.value,
        autoApplyResults: !!el('autoApplyResults')?.checked,
        autoCreateRounds: !!el('autoCreateRounds')?.checked,
        newRoundDefaultMode: el('newRoundDefaultMode')?.value,
        allowManualOverride: !!el('allowManualOverride')?.checked,
        updatedAt: new Date().toISOString()
      });
    }

    function refreshSummary() {
      el('automationSettingsSummary').innerHTML = settingsSummary(readSettingsForm());
    }

    async function applySettingsToRounds(showToastAfter = true) {
      ensureStateSettings();
      const settings = getAutomationSettings();
      let changed = 0;

      state.rounds.forEach(round => {
        if (!round?.matchTime) return;
        if (round.manualState && round.manualState !== 'auto') return;

        const times = computeAutomationTimes(round.matchTime, settings);
      ...
      persistLocalState();
      if (firebaseDbRef) await firebaseDbRef.child('rounds').set(state.rounds);
      if (showToastAfter) showToast(`${changed} rodada(s) automática(s) atualizada(s).`);
    }

    pane.addEventListener('input', refreshSummary);
    pane.addEventListener('change', refreshSummary);

    el('automationSettingsForm')?.addEventListener('submit', async event => {
      event.preventDefault();
      try {
        ensureStateSettings();
        state.settings.automation = readSettingsForm();
        persistLocalState();
        if (firebaseDbRef) await firebaseDbRef.child('settings/automation').set(state.settings.automation);
        await applySettingsToRounds(false);
        showToast('Configurações salvas e aplicadas às rodadas automáticas.');
        renderAll('admin');
      } catch (err) {
        console.error('Erro ao salvar configurações:', err);
        showToast('Erro ao salvar configurações: ' + (err?.code || err?.message || err));
      }
    });

    el('automationApplyRoundsBtn')?.addEventListener('click', async () => {
      try {
        await applySettingsToRounds(true);
        renderAll('admin');
      } catch (err) {
        console.error('Erro ao aplicar rodadas:', err);
        showToast('Erro ao aplicar nas rodadas: ' + (err?.code || err?.message || err));
      }
    });
  }

  const originalRenderAdmin = renderAdmin;
  renderAdmin = function renderAdminV6() {
    originalRenderAdmin();
    renderAutomationSettingsPanel();
    setTimeout(() => renderAdminRoundStatusRibbon(), 0);
  };

  const originalSetupEvents = setupEvents;
  setupEvents = function setupEventsV6() {
    originalSetupEvents();

    ['roundSelect', 'roundManualState', 'roundMatchTime', 'roundDeadline', 'resultCruzeiro', 'resultOpponent'].forEach(id => {
      const node = el(id);
      if (!node || node.dataset.repairV6Bound) return;
      node.dataset.repairV6Bound = '1';
      node.addEventListener('input', () => renderAdminRoundStatusRibbon());
      node.addEventListener('change', () => renderAdminRoundStatusRibbon());
    });
  };

  const originalInit = init;
  init = async function initV6() {
    await originalInit();
    ensureStateSettings();
  };
})();
/* === END BOLAO CLEAN REPAIR V6 === */
