...
function computeAutomationTimes(matchTime, settings = getAutomationSettings()) {
  ...
  if (!matchMs || Number.isNaN(matchMs)) {
...
function buildInitialState() {
  return {
    ...
    settings: { automation: { ...DEFAULT_AUTOMATION_SETTINGS } }
  };
}
...
function saveState(scope = 'all') {
  ...
  return writePromise.catch(err => {
    ...
    showToast('Erro ao salvar no Firebase. Veja o UID no painel.');
  });
}
...
function populateRoundForm(roundId) {
  ...
  const displayDeadline = (round.manualState || 'auto') === 'auto'
    ? (times.deadline || round.deadline || '')
    : (round.deadline || '');
  ...
  el('roundDeadline').value = displayDeadline;
...
function setupEvents() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (targetId === 'tab-admin-settings') renderAutomationSettingsPanel();
  });
  ...
  el('roundForm').addEventListener('submit', (e) => {
    ...
    const computedTimes = computeAutomationTimes(matchTimeVal);
    round.autoOpenAt     = computedTimes.autoOpenAt || round.autoOpenAt || '';
    round.deadline       = manualStateVal === 'auto' ? (computedTimes.deadline || deadlineVal) : deadlineVal;
  ...
  el('newRoundBtn').addEventListener('click', () => {
    ...
    const deadline  = computeAutomationTimes(matchTime).deadline;
    ...
    const round = {
      ...
      resultCruzeiro: null,
      resultOpponent: null,
      manualState: getAutomationSettings().newRoundDefaultMode || 'auto',
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };
...
function ensureAutomationStyles() {
  ...
  style.textContent = `
    ...
    .admin-auto-status-ribbon.result .admin-auto-status-title { color: var(--gold); }
    .admin-auto-status-ribbon.manual {
      border-color: rgba(255,125,125,.40);
      background: rgba(255,125,125,.10);
    }
    .admin-auto-status-ribbon.manual .admin-auto-status-title { color: var(--red); }
    ...
    .automation-card-title strong { font-size:1rem; }
    ...
    @media (max-width:720px) {
      ...
      .automation-hint { padding-bottom:0; }
      .automation-toggle-row { align-items:flex-start; }
    }
  `;
  document.head.appendChild(style);
}

function renderAdminRoundStatusRibbon(round = getRound(el('roundSelect')?.value)) {
  ...
  if (!ribbon) {
    ...
    ribbon.className = 'admin-auto-status-ribbon';
  ...
  const draft = {
    ...round,
    matchTime: el('roundMatchTime')?.value || round.matchTime,
    deadline: el('roundDeadline')?.value || round.deadline,
    manualState: el('roundManualState')?.value || round.manualState || 'auto',
    resultCruzeiro: el('resultCruzeiro')?.value === '' ? round.resultCruzeiro : Number(el('resultCruzeiro')?.value),
    resultOpponent: el('resultOpponent')?.value === '' ? round.resultOpponent : Number(el('resultOpponent')?.value)
  };
  ...
  if (hasResult) {
  ...
  } else if (settings.allowManualOverride !== false && draft.manualState && draft.manualState !== 'auto') {
    const labels = {
      upcoming: 'Em espera',
      open: 'Apostas abertas',
      closed: 'Apostas encerradas',
    ...
    const matchMs = parseAppDateTime(draft.matchTime);
    const openMs = matchMs - automationUnitMs(settings.openBeforeValue, settings.openBeforeUnit);
    ...
    if (!matchMs || Number.isNaN(matchMs)) {
      ...
      const mins = Math.floor((diff % 3600000) / 60000);
      cls = 'waiting';
      title = `Apostas abrem em ${hours}h ${mins}min`;
      ...
      desc = settings.closeBetsEnabled
        ? `Fecham em ${hours}h ${mins}min, às ${formatDateTime(toLocalInputInAppTime(new Date(closeMs)))}.`
...
function automationUnitOptions(selected) {
...
function automationSummaryHTML(settings) {
  ...
  const unitLabel = unit => unit === 'hours' ? 'hora(s)' : unit === 'days' ? 'dia(s)' : 'minuto(s)';

  return [
    settings.openBetsEnabled
      ? `As apostas abrem ${settings.openBeforeValue} ${unitLabel(settings.openBeforeUnit)} antes do jogo.`
      : 'A abertura automática das apostas está desligada.',
    settings.closeBetsEnabled
      ? `As apostas fecham ${settings.closeBeforeValue} ${unitLabel(settings.closeBeforeUnit)} antes do jogo.`
      : 'O fecho automático das apostas está desligado.',
    settings.autoApplyResults
...
function automationSettingsFormHTML(settings) {
  return `
    <section class="panel">
      ...
      <form id="automationSettingsForm" class="automation-settings-grid">
        ...
        <div class="automation-settings-card">
          ...
          <div class="automation-row">
            ...
            <label class="automation-field">
              Unidade
              <select id="openBeforeUnit">${automationUnitOptions(settings.openBeforeUnit)}</select>
            </label>
          ...
          <div class="automation-row">
            ...
            <label class="automation-field">
              Unidade
              <select id="closeBeforeUnit">${automationUnitOptions(settings.closeBeforeUnit)}</select>
            </label>
            <span class="automation-hint">antes do jogo</span>
          </div>
        </div>
...
async function saveAutomationSettings(settings) {
  ...
  state.settings.automation = normalizeAutomationSettings({
    ...settings,
    updatedAt: new Date().toISOString()
  });
  ...
  if (firebaseDbRef) {
    await firebaseDbRef.child('settings/automation').set(state.settings.automation);
  }

  return state.settings.automation;
}

async function applyAutomationToAutomaticRounds(showToastAfter = true) {
  ensureStateSettings();
  ...
  if (firebaseDbRef) {
    await firebaseDbRef.child('rounds').set(state.rounds);
  }

  if (showToastAfter) {
    showToast(`${changed} rodada(s) automática(s) atualizada(s).`);
  }
}

function refreshAutomationSettingsSummary() {
  const summary = el('automationSettingsSummary');
  if (!summary) return;
  summary.innerHTML = automationSummaryHTML(readAutomationSettingsForm());
}
...
function renderAutomationSettingsPanel() {
  ...
  if (!pane.dataset.rendered) {
    ...
    pane.innerHTML = automationSettingsFormHTML(getAutomationSettings());

    pane.addEventListener('input', refreshAutomationSettingsSummary);
    pane.addEventListener('change', refreshAutomationSettingsSummary);

    el('automationSettingsForm')?.addEventListener('submit', async event => {
      event.preventDefault();

      try {
        const saved = await saveAutomationSettings(readAutomationSettingsForm());
        await applyAutomationToAutomaticRounds(false);
        el('automationSettingsSummary').innerHTML = automationSummaryHTML(saved);
    ...
    el('automationResetBtn')?.addEventListener('click', async () => {
      try {
        const saved = await saveAutomationSettings(DEFAULT_AUTOMATION_SETTINGS);
        await applyAutomationToAutomaticRounds(false);
        pane.dataset.rendered = '';
        pane.innerHTML = automationSettingsFormHTML(saved);
        showToast('Configurações restauradas.');
        renderAll('admin');
      } catch (err) {
        console.error(err);
        showToast('Erro ao restaurar configurações.');
      }
    });

    el('automationApplyRoundsBtn')?.addEventListener('click', async () => {
      if (!confirm('Aplicar abertura e fecho às rodadas que estão em modo Automático?')) return;

      try {
        await applyAutomationToAutomaticRounds(true);
        renderAll('admin');
      } catch (err) {
        console.error(err);
        showToast('Erro ao aplicar nas rodadas.');
      }
    });
  }
}
